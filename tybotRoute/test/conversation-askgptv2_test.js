...
describe('Conversation for AskGPTV2 test', async () => {
  ...
  it('/gpt_success_gemini (key from integrations) - invokes the askgpt mockup and test the returning attributes', (done) => {
    let listener;
    let endpointServer = express();
    endpointServer.use(bodyParser.json());
    endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
    ...
    listener = endpointServer.listen(10002, '0.0.0.0', () => {
      ...
      let request = {
        "payload": {
          ...
          "sender": "A-SENDER",
          "recipient": REQUEST_ID,
          "text": '/kb_success_gemini{"last_user_message":"come ti chiami"}',
          "id_project": PROJECT_ID,
          "metadata": "",
          "request": {
            "request_id": REQUEST_ID
  ...
  it('/gpt_success_pinecone_reranking - invokes the askgpt mockup and test the returning attributes', (done) => {
    let listener;
    let endpointServer = express();
    endpointServer.use(bodyParser.json());
    endpointServer.post('/:projectId/requests/:requestId/messages', function (req, res) {
      ...
      const command2 = message.attributes.commands[1];
      assert(command2.type === "message");
      assert(command2.message.text === "kb replied: this is mock kb reply with pinecone reranking");
      ...
      util.getChatbotParameters(REQUEST_ID, (err, attributes) => {
        if (err) {
          ...
          assert(attributes);
          assert(attributes["kb_reply"] === "this is mock kb reply with pinecone reranking")
          listener.close(() => {
            done();
          });
        }
    ...
    endpointServer.post('/api/qa', function (req, res) {
      ...
      assert(req.body.reranking !== null);
      assert(typeof req.body.reranking === "object");
      assert(req.body.reranking.provider === "pinecone");
      assert(req.body.reranking.api_key === process.env.PINECONE_API_KEY);
      assert(req.body.reranking.model === process.env.PINECONE_RERANKING_MODEL || process.env.RERANKING_MODEL || "bge-reranker-v2-m3");
      ...
      if (!req.body.question) {
        ...
        reply = {
          answer: "this is mock kb reply with pinecone reranking",
          success: true,
          id: "123456789",
          ids: ["9876543210", "0123456789"],
          source: "http://gethelp.test.com/article",
    ...
    listener = endpointServer.listen(10002, '0.0.0.0', () => {
      ...
      let request = {
        "payload": {
          ...
          "sender": "A-SENDER",
          "recipient": REQUEST_ID,
          "text": '/kb_success_pinecone_reranking{"last_user_message":"come ti chiami", "custom_context":"sei un assistente fantastico"}',
          "id_project": PROJECT_ID,
          "metadata": "",
          "request": {
            "request_id": REQUEST_ID
